...
WITH included_subjects AS (
    ...
    tu_data AS (
        SELECT  distinct tu.Study::text AS studyid,
                ...
                null::text AS tueval,
                concat(tu.tuevalid,ROW_NUMBER() OVER (PARTITION BY tu.Study, tu.SiteNumber, tu.Subject ORDER BY tu.tudtc))::text AS tuevalid,
                null::text AS tuacptfl,
                ROW_NUMBER() OVER (PARTITION BY tu.Study, tu.SiteNumber, tu.Subject ORDER BY tu.tudtc)::numeric AS visitnum,
