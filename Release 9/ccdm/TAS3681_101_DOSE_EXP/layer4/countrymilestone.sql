...
WITH included_studies AS (
 ...
 site_region_config AS
		(
		  SELECT 	UPPER(alpha_2_code)::text AS alpha_2_code,
					UPPER(alpha_3_code)::text AS alpha_3_code,
					UPPER(country_name)::text AS country_name,
					siteregion::text AS siteregion
		  FROM internal_config.site_region_config
		),
		
taiho_ms_standards AS (select * from internal_config.taiho_ms_standards where milestonelevel='Country')	,
