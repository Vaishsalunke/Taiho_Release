...
tv_scheduled AS (
	...
	SELECT visit_number     as      visitnum, visit, visitdy , window_before_visit_day as "visitwindowbefore", window_after_visit_day as "visitwindowafter"
            FROM tv_internal.tv_tracker_list where studyid='TAS117-201'
            and (visit,visit_number) in (select visit,max(visit_number) from tv_internal.tv_tracker_list where studyid='TAS117-201' group by 1)
