...
WITH included_studies AS (
    ...
    fielddata_data AS (
        SELECT f.studyid AS studyid,
			...
			case when f.visit_cnt>1 then REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(f.visit,'<WK[0-9]D[0-9]/>',''),'\s\([0-9]\)',''),'<WK[0-9]D[0-9][0-9]/>',''),'-[0-9]','')||'-'||f.InstanceRepeatNumber 
             else REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(f.visit,'<WK[0-9]D[0-9]/>',''),'\s\([0-9]\)',''),'<WK[0-9]D[0-9][0-9]/>',''),'-[0-9]','') end AS visit,
