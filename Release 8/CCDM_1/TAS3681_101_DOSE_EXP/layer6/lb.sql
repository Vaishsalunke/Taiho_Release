...
SELECT DISTINCT studyid, siteid, sitename, sitecountry,sitecountrycode, siteregion FROM site),	
    ...
    (
        ...
        FROM
            tas3681_101_lab."NormLab" lb1
        LEFT JOIN
            tas3681_101."CHEM" chem
        ON
            (
   ...
   SELECT studyid,siteid,usubjid,dsstdtc FROM ds WHERE dsterm = 'Enrolled'
    ...
    (
        SELECT
            ...
            (REGEXP_REPLACE(REGEXP_REPLACE(REPLACE(REPLACE(visit,'(1)',''),'  Cycle',' Cycle'),'<W[0-9]DA[0-9]/>\sExpansion',''
...
SELECT
	...
	null::text							  AS timpnt	
    /*KEY , (lb.studyid || '~' || lb.siteid || '~' || lb.usubjid || '~' || lb.lbseq)::text  AS
    objectuniquekey KEY*/
