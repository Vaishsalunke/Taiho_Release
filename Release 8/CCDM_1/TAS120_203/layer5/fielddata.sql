...
WITH included_studies AS (
    ...
    fielddata_data AS (
        SELECT f.studyid AS studyid,
            concat('TAS120_203_',split_part(f.siteidjoin,'_',2)) AS siteidjoin,
            f.usubjid AS usubjid,
            f.formid AS formid, 
            f.formseq AS formseq,
            f.fieldid AS fieldid,
            f.fieldseq AS fieldseq,
			case when f.visit_cnt>1 then REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(f.visit,'<WK[0-9]D[0-9]/>',''),'<WK[0-9]D[0-9][0-9]/>',''),'-[0-9]','')||'-'||f.InstanceRepeatNumber 
             else REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(f.visit,'<WK[0-9]D[0-9]/>',''),'<WK[0-9]D[0-9][0-9]/>',''),'-[0-9]','') end AS visit,
