...
WITH included_subjects AS (
     ...
     query_data AS (
                SELECT  substring(study,1,position ('-' in study)-2)::text AS studyId,
                        'TAS120_203'::text AS studyname,
                        concat(substring(study,1,position ('-' in study)-2),'_',split_part(sitename,'_',1))::text AS siteId,
                        ...
                        from tas120_203.stream_query_detail sqd ),
