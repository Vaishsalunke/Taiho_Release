...
WITH included_subjects AS (
     ...
     mh_data AS (
                SELECT  project::text AS studyid,
                        ...
                        concat("PageRepeatNumber" ,"RecordPosition") ::int AS mhseq, /*(row_number() over (partition by [studyid],[siteid],[usubjid] order [mhstdtc,mhsttm]))::int AS mhseq,*/
                        null::text AS mhspid,
                        "MHTERM"::text AS mhterm,
                        nullif("MHTERM_PT",'')::text AS mhdecod,
                        nullif("MHTERM_SOC",'')::text AS mhcat,
                        'Medical History'::text AS mhscat,
                        nullif("MHTERM_SOC_CD",'')::text AS mhbodsys,
