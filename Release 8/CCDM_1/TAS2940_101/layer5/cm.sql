...
WITH included_subjects AS (
     ...
     cm_data AS (
                SELECT  "project"::text AS studyid,
                        ...
                        concat("PageRepeatNumber" ,"RecordPosition") ::integer AS cmseq,
                        null::text AS cmspid,
                        "CMTRT"::text AS cmtrt,
                        nullif("CMTRT_TN",'')::text AS cmmodify,
                        nullif("CMTRT_PT",'')::text AS cmdecod,
                        nullif("CMTRT_ATC",'')::text AS cmcat,
                        ...
                        CASE WHEN "CMROUTE"='Other' then  "CMROUTE" || "CMROUTEOTH"   ELSE  "CMROUTE"end::text AS cmroute,
...
left join site_data sd on (cm.studyid = sd.studyid AND cm.siteid = sd.siteid);
