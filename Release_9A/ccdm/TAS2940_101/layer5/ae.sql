...
WITH included_subjects AS (
     ...
     ae_data AS (
                SELECT distinct "project" ::text AS studyid,
                        ...
                        case when "AEACTSN" = 1 then 'None'
						 when "AEACTSDR" = 1 then 'Dose Reduced'
						 when "AEACTSID" = 1 then 'Interrupted/Delayed'
						 when "AEACTSDQ" = 1 then 'Discontinued' end::text as aeacn,
                        "AEOUT" ::text AS aeout,
                        null::text AS aetox,
