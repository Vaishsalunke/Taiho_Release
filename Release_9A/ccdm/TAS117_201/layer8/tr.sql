...
WITH included_subjects AS (
    ...
    tr_data AS (
        select distinct tr.comprehendid,
						replace(tr.studyid,'TAS117_201','TAS-117-201') as studyid,
						tr.siteid,
						tr.usubjid,
						...
						tr.trrefid,
						tr.trspid,
						COALESCE(trlnkid,row_number() over(partition by tr.studyid, tr.siteid,tr.usubjid order by trdtc)::text)::text as trlnkid,
						tr.trlnkgrp,
						tr.trtestcd,
						tr.trtest,
						tr.trorres,
		...
		from
				(
	                ...
	                'mm'::text AS trstresu,
	                case when
	                tlb."MEASURMT" is null then 'NOT DONE'
	                else null
	                end ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
