...
WITH included_subjects AS (
    ...
    tu_data AS (
        ...
        SELECT  distinct Study::text AS studyid,
                ...
                --concat(tuevalid,ROW_NUMBER() OVER (PARTITION BY Study, SiteNumber, Subject ORDER BY tudtc))::text AS 
		...
		FROM	(
					...
					Select 		"project":: text as Study, 
								...
								coalesce(nullif("NTLNUM",''),"RecordId"::text):: text as tulnkid,
								"RecordPosition"::text as tulnkgrp,
								'Lesion ID'::text AS tutestcd,
								'Lesion Identification':: text as tutest,
					...
					Select 		"project":: text as Study, 
								...
								coalesce(nullif("TLNUM",''),"RecordId"::text):: text as tulnkid,
								"RecordPosition"::text as tulnkgrp,
								'Lesion ID'::text AS tutestcd,
								'Lesion Identification':: text as tutest,
		...
		left join sv
on 'TAS2940_101'=sv.studyid and tu.SiteNumber=sv.siteid and tu.Subject=sv.usubjid
