...
WITH included_subjects AS (
    ...
    tu_data AS (
        ...
        tu.turefid,
        tu.tuspid,
        coalesce (concat(tugrpid ,' - ', tulnkgrp), (row_number() over(partition by tu.studyid, tu.siteid,tu.usubjid order by tudtc))::text) as tulnkid,
        tu.tulnkgrp,
        tu.tutestcd,
        tu.tutest,
        tu.tuorres,
        ...
        tu.tublfl,
        tu.tueval,
        --concat(tu.tuevalid,ROW_NUMBER() OVER (PARTITION BY tu.studyid, tu.siteid, tu.usubjid ORDER BY tudtc)) as 
		tuevalid,
        tu.tuacptfl,
