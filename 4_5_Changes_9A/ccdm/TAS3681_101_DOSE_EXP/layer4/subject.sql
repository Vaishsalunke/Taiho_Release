    ...
    subject_data AS (
                SELECT  distinct 'TAS3681_101_DOSE_EXP'::text AS studyid,
						...
						nullif(COALESCE(	case
								 when dm."DataPageName" = 'Demography-Expansion' then 'Dose Expansion'||' '||s."IEEXPGRP"
								 else dlt."DLTCOH"||' '||s."IEEXPGRP"
							 end,
					         case
								 when dm2."DataPageName" = 'Demography-Expansion' then 'Dose Expansion'||' '||s."IEEXPGRP"
								 else dlt."DLTCOH"||' '||s."IEEXPGRP"::text
							 end),'')::text AS arm,
