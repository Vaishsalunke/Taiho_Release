...
WITH included_subjects AS (
    ...
    tr_data AS (
        select distinct tr.comprehendid,
						replace(tr.studyid,'TAS120_203','TAS-120-203')::text AS studyid,
						tr.siteid,
						tr.usubjid,
		...
		from
				...
				SELECT  	distinct	null::text AS comprehendid,
                	...
                	null::text AS trrefid,
                	null::text AS trspid,
                	'NTL'||ntlb."NTLBID" ::text AS trlnkid,
                	'1' ::text AS trlnkgrp,
                	'TUMSTATE'::text AS trtestcd,
                	'Tumor State'::text AS trtest,
                	'Present'::text AS trorres,
	                ...
	                'mm'::text AS trstresu,
	               case 
	               	when "TLSMM" = 1 then 'Too Small To Measure' 
