        ...
        FROM
            (
                SELECT DISTINCT
                    ...
                    'TAS120_203_' || split_part(lb1."SiteNumber",'_',2)::text AS siteid,
            ...
            (lb1."InstanceName",'\s\([0-9][0-9]\)','')
                    ...
                    CASE
                        ...
                        THEN MAX(coag."LBDAT")
                         WHEN lb1."DataPageName" LIKE '%Thyroid Function Test%'
                        THEN MAX(tft."TFTDAT")
                ...
                FROM
                    tas120_203_lab."NormLab" lb1
                LEFT JOIN
                    tas120_203."CHEM" chem
                ON
                    (
                ...
                ON
                    (
                        lb1."project" = tft."project"
                    AND lb1."SiteNumber" = tft."SiteNumber"
                    AND lb1."Subject" = tft."Subject"
                    AND lb1."InstanceName" = tft."InstanceName")
                GROUP BY
...
final_lb as
        (
        select  distinct  replace(lb.studyid,'TAS120_203','TAS-120-203') as studyid,
                    lb.siteid,
                    lb.usubjid,
                    lb.visit,
                    lbdtc,
