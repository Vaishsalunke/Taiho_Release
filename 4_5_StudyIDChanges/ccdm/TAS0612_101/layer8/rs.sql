...
WITH included_subjects AS (
    ...
    rs_data AS (select *,case when rsdtc1::date-prev_visit_date::date is not null then concat((rsdtc1::date-prev_visit_date::date)::numeric,' Days') end::text AS rsevlint
    			 from(
        select distinct replace(u.studyid,'TAS0612_101','TAS0612-101') as studyid,
                        u.siteid,
						u.usubjid,						
						rsseq,
						null as rsgrpid,
				...
				(
					select distinct	project::text AS studyid,
									...
									row_number()over(partition by project,or1.siteid,"Subject" order by "ORDAT")::numeric AS rsseq,
									(dense_rank () over (partition by project, concat('TAS0612_101_',split_part("SiteNumber",'_',2)), "Subject" order by "ORDAT")::numeric+1)::text AS rslnkgrp,
				 ...
				 left join sv on or1."FolderName" = sv.visit and or1."ORDAT" ::date = sv.svstdtc 
					...
					("ORTLRES",'TRGRESP','Target Response',"ORTLRES","ORTLRES_STD",case when nullif("ORTLRES",'') is not null then null else 'Not Done' end),
