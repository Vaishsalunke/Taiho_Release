...
WITH included_subjects AS (
    ...
    tr_data AS (
       select distinct u.comprehendid,
		...
		from
				(
					...
					SELECT  			
									...
									"TLDIM"::numeric AS trstresn,
									'mm'::text AS trstresu,
									case
										when lower("TLTSTM") = 'not assessed' then 'Too Small To Measure'
										else "TLYN"
									end
									::text AS trstat,--------------------3
									null::text AS trreasnd,
		...
		left join sv on sv.visit = u.visit and u.trdtc::date = sv.svstdtc  
                )
