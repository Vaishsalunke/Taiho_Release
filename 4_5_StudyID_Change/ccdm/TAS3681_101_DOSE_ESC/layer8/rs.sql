...
WITH included_subjects AS (
    ...
    rs_data AS ( select *, case when rsdtc1::date-prev_visit_date::date is not null then concat((rsdtc1::date-prev_visit_date::date)::numeric,' Days') 					   
        ...
        select distinct rs.comprehendid,
						...
						rsspid,
						rslnkid,
						dense_rank () over (partition by rs.studyid,rs.siteid,rs.usubjid order by rs.rsdtc)::numeric + 1 as rslnkgrp,
						rstestcd,
						rstest,
						rscat,
						rsscat,
		...
		from(
				...
				from tas3681_101."OR" or1
				left join sv
					on  or1."FolderName"  = sv.visit and or1."ORDAT" = sv.svstdtc 
				...
				CROSS JOIN LATERAL(VALUES
									...
									"ORTLRES_STD",case when nullif("ORTLRES",'') is not null then null else 'Not Done' end),
									("ORNTLRES",'NTRGRESP','Non-Target Response',"ORNTLRES",
									"ORNTLRES_STD",case when nullif("ORNTLRES",'') is not null then null else 'Not Done' end),
									("ORRES",'OVRLRESP','Overall Response',"ORRES",
									"ORRES_STD",case when nullif("ORRES",'') is not null then null else 'Not Done' end)
