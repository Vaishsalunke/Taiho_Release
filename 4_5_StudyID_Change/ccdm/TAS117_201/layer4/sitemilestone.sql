...
WITH included_sites AS (
	 ...
	 SELECT  		distinct	'TAS117-201'::text AS studyid,
                        ...
                        concat('TAS117_201_',split_part (sm."site_number",'_',2))::text AS siteid,
                        tms.milestoneseq::int AS milestoneseq,
                        replace (tms.start_standard_label,'1st Subject 1st Treatment','First Subject In')::text AS milestonelabel,
					...
					from tas117_201_ctms.site_milestones sm
	...
	Select sm.*,tms.endmilestonecycletime_standard:: text as endmilestonecycletime from(
					select distinct	'TAS117-201'::text AS studyid,
						'TAS117_201'::text AS studyname,
                        concat('TAS117_201_',split_part (d."SiteNumber" ,'_',2))::text AS siteid,
                        999::int AS milestoneseq,
                        'FIRST SUBJECT IN'::text AS milestonelabel,
                        'Actual'::text AS milestonetype,
                         min(d."DADAT")::date AS expecteddate,
                        'yes'::boolean AS ismandatory,
...
union	
					...
					select distinct	'TAS117-201'::text AS studyid,
						'TAS117_201'::text AS studyname,
                        concat('TAS117_201_',split_part (s."site_number",'_',2))::text AS siteid,
