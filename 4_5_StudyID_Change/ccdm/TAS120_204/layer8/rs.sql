...
WITH included_subjects AS (
	...
	rs_data AS ( select *,case when rsdtc1::date-prev_visit_date::date is not null then concat((rsdtc1::date-prev_visit_date::date)::numeric,' Days') 					   
                ...
                (dense_rank () over (partition by 'TAS-120-204', concat(project,'_',split_part("SiteNumber",'_',2)), "Subject" order by "ORDAT")::numeric+1)::text AS rslnkgrp,
