...
WITH included_subjects AS (SELECT DISTINCT studyid, siteid, usubjid FROM subject),
    ...
    lb_data AS
        ...
        SELECT
            ...
            (REGEXP_REPLACE(REGEXP_REPLACE(REPLACE(lb.visit,'(1)',''),'<W[0-9]DA[0-9]/>\sExpansion',''
           ...
           /* (row_number() over (partition BY lb.studyid, lb.siteid, lb.usubjid ORDER BY lb.lbtestcd
            ...
            lbseq,
            folderseq,
            COALESCE(lb.lbtestcd,'') AS lbtestcd,
            lb.lbtest                AS lbtest,
            lb.lbcat,
            lbscat,
            lbspec,
...
final_lb as
        (
        select  distinct  replace(lb.studyid,'TAS120_202','TAS-120-202') as studyid,
                    lb.siteid,
                    lb.usubjid,
                    lb.visit,
                    lbdtc,
