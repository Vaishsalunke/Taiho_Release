...
/*ex_data as (
    ...
    tr_data AS (
        select distinct tr.comprehendid,
						replace(tr.studyid,'TAS120_202','TAS-120-202') as studyid,
						tr.siteid,
						tr.usubjid,
						...
						trrefid,
						trspid,
						coalesce (concat(trgrpid ,' - ', trlnkgrp), (row_number() over(partition by tr.studyid, tr.siteid,tr.usubjid order by trdtc))::text) as trlnkid,
						trlnkgrp,
						trtestcd,
						trtest,
						trorres,
		...
		from
				...
				SELECT  	distinct	null::text AS comprehendid,
                	...
                	null::text AS trspid,
                	'NL'||nl."RecordPosition"::text AS trlnkid,
                	(row_number() over (partition by project,"SiteNumber", "Subject" order by "NLDAT")::numeric + 1) ::text AS trlnkgrp,
                	...
                	null::text AS trspid,
                	'NTL'||"RecordPosition"::text AS trlnkid,
                	(row_number() over (partition by project,"SiteNumber" , "Subject" order by "NTLDAT")::numeric + 1) ::text AS trlnkgrp,
                	'TUMSTATE'::text AS trtestcd,
                	'Tumor State'::text AS trtest,
                	"NTLBSTAT"::text AS trorres,
	                'mm'::text AS trorresu,
	                "NTLBSTAT" ::text AS trstresc,
	                1::numeric  AS trstresn,
	                'mm'::text AS trstresu,
	                case 
	                	when "NTLNA" = 1 then 'Not Done' else null
	                end
	                ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
                	...
                	(row_number() over (partition by project,"SiteNumber", "Subject" order by "TLDAT")::numeric + 1) ::text AS trlnkgrp,
	                ...
	                'mm'::text AS trstresu,
	                case 
		                when "TLSMALL" =1 then 'Too Small To Measure'
		                when "TLNA" = 1 then 'Not Done'
	                end ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
	                ...
	                'mm'::text AS trstresu,
	                case when
	                tlb."TLBDIM" is null then 'NOT DONE'
	                else null
	                end ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
