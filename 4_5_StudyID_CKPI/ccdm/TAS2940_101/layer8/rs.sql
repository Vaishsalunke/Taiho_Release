...
WITH included_subjects AS (
	...
	rs_data AS ( select *,case when rsdtc1::date-prev_visit_date::date is not null then concat((rsdtc1::date-prev_visit_date::date)::numeric,' Days') end::text AS rsevlint
                ...
                row_number()over (partition by 'TAS2940-101',concat(project,'_',split_part("SiteNumber",'_',2)),"Subject" order by "ORDAT")::numeric AS rsseq,
                null::text AS rsgrpid,
                null::text AS rsrefid,
                null::text AS rsspid,
                null::text AS rslnkid,
                (dense_rank () over (partition by 'TAS2940-101', concat(project,'_',split_part("SiteNumber",'_',2)), "Subject" order by "ORDAT")::numeric+1)::text AS rslnkgrp,
                ...
                to_char("ORDAT"::date,'DD-MM-YYYY')::text AS rsdtc,
			...
			left join sv on or1."FolderName" = sv.visit and or1."ORDAT"::date = sv.svstdtc  
						...
						CROSS JOIN LATERAL(values 
												...
												("ORRES",'OVRLRESP' ,'Overall Response',case when "ORNLYN" = 'Yes' then 'New lesion' else '' end ,case when "ORNLYN" = 'Yes' then 'New lesion' else '' end,case when nullif("ORRES",'') is not null then null else 'Not Done' end )
