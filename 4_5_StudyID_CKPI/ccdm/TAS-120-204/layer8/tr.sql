...
WITH included_subjects AS (
	...
	tr_data AS (
				select distinct tr.comprehendid,
						replace(tr.studyid,'TAS120_204','TAS-120-204') as studyid,
						tr.siteid,
						tr.usubjid,
		...
		from
				...
				SELECT  	distinct	null::text AS comprehendid,
	                ...
	                'mm'::text AS trstresu,
	                case 
	                	when ntl."NTLNA" = 1 then 'Not Done'
	                end
	                ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
	                ...
	                'mm'::text AS trstresu,
	               case 
	               	when tl."TLSMALL" = 1 then 'Too Small To Measure'
	               	when tl."TLNA" = 1 then 'Not Done'
	               	else null
	               end
	               ::text AS trstat,
	                null ::text AS trreasnd,
			...
			left join dm								
			on 'TAS120_204' = dm."studyid" and tr.siteid  = dm.siteid and tr.usubjid = dm."usubjid"
			...
			left join sv on tr.visit = sv.visit and 'TAS-120-204' = sv.studyid and tr.siteid = sv.siteid and tr.usubjid = sv.usubjid -- done
