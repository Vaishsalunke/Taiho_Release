...
WITH included_subjects AS (
    ...
    select distinct tr.comprehendid,
						replace(tr.studyid,'TAS0612_101','TAS0612-101') as studyid,
						tr.siteid,
						tr.usubjid,
		...
		from
				...
				SELECT  	distinct	null::text AS comprehendid,
	                ...
	                1::numeric  AS trstresn,
	                'mm'::text AS trstresu,
	                case when lower(ntlb."NTLBYN") = 'yes' then 'Done' else null end ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
	                ...
	                'mm'::text AS trstresu,
	                case 
	                	when ntl."NTLNA" = 1 then 'Not Done' else null
	                end
	                ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
	                ...
	                'mm'::text AS trstresu,
	               case when tl."TLNA" = 1 then 'Not Done' else null end ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
	                ...
	                'mm'::text AS trstresu,
	                case when lower(tlb."TLBSTAT") = 'yes' then 'Done' else null end ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
		...
		left join sv on tr.visit = sv.visit and 'TAS0612-101' = sv.studyid and tr.siteid = sv.siteid and tr.usubjid = sv.usubjid -- done
