...
SELECT DISTINCT studyid, siteid, sitename, sitecountry,sitecountrycode, siteregion FROM site),	
     ...
     ae_sev_toxgr_data AS (
                SELECT  'TAS3681_101_DOSE_EXP'::text AS studyid,
                        ...
                        CASE
                        WHEN("AEONSGR")= '0' THEN 'G0'
			WHEN("AEONSGR")= '1' THEN 'G1'
		        WHEN("AEONSGR")= '2' THEN 'G2'
			WHEN("AEONSGR")= '3' THEN 'G3'
			WHEN("AEONSGR")= '4' THEN 'G4'
			WHEN("AEONSGR")= '5' THEN 'G5'								   
		        WHEN(("AEONSGR")= '' OR ("AEONSGR") IS NULL) THEN 'Missing'
			END::text AS faorres,
