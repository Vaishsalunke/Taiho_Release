...
WITH included_subjects AS (SELECT DISTINCT studyid, siteid, usubjid FROM subject),
        ...
        FROM
            (
                SELECT DISTINCT
                    ...
                    'TAS2940_101_' || split_part(lb1."SiteNumber",'_',2)::text AS siteid,
    ...
    (lb1."InstanceName",'\s\([0-9][0-9]\)','')
                ...
                FROM
                    TAS2940_101_lab."NormLab" lb1
                LEFT JOIN
                    TAS2940_101."CHEM" chem
                ON
                    (
...
final_lb as
        (
        select  distinct  replace(lb.studyid,'TAS2940_101','TAS2940-101') as studyid,
                    lb.siteid,
                    lb.usubjid,
                    lb.visit,
                    lbdtc,
