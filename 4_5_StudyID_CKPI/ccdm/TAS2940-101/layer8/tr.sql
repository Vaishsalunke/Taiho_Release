...
WITH included_subjects AS (
    ...
    select distinct tr.comprehendid,
						replace(tr.studyid,'TAS2940_101','TAS2940-101') as studyid,
						tr.siteid,
						tr.usubjid,
		...
		from
				...
				SELECT  	distinct	null::text AS comprehendid,
	                ...
	                'mm'::text AS trstresu,
	                 case when tl."TLSMALL" = 1 then 'Too Small To Measure' when tl."TLNA" = 1 then 'Not Done' else null end ::text AS trstat,
	                null ::text AS trreasnd,
	                null::text AS trnam,
